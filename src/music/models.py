...
class TracksModel(models.Model):
    ...
    track_audioFile = models.FileField(null=False, blank=True, upload_to=Track_Files, validators=[validators.validate_audio_extension])
...
class FavouritesModel(models.Model):
    ...
    def __str__(self):
        return f"{self.track_id}: {self.user_FUI}"
...
class PurchasedAlbumsModel(models.Model):
    ...
    def __str__(self):
        return f"{self.album_id}: {self.user_FUI}"
...
class AdminCollectionTracksModel(models.Model):
    ...
    def __str__(self):
        return f"{self.collection_id}: {self.track_id}"